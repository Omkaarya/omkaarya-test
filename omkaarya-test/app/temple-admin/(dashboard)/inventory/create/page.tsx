...
const TYPES: { id: ProductType; ico: string; name: string; desc: string }[] = [
  { id: "Consumable", ico: "🔁", name: "Consumable", desc: "Prasad, flowers, incense, oil" },
  { id: "Equipment", ico: "⚙️", name: "Equipment", desc: "Lamps, vessels, garments" },
  { id: "Sale Item", ico: "🛒", name: "POS / Sale item", desc: "Prasad packets, kits, books" },
  { id: "Admin", ico: "🗃️", name: "Office & Admin", desc: "Stationery, cleaning" },
  { id: "Festival", ico: "🎪", name: "Festival stock", desc: "Festival-specific items" },
];
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-5">
        ...
        <div className="grid grid-cols-2 gap-3.5">
          <div className="flex flex-col gap-1">
            ...
            <input className={inputCls} placeholder="e.g. Besan Ladoo, Brass Lamp 5-wick" />
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Also known as</label>
            <input className={inputCls} placeholder="Alternate or regional names" />
          </div>
          <div className="flex flex-col gap-1">
            ...
            <select className={selectCls}>
              ...
              <optgroup label="Consumables">
                <option>Prasad</option><option>Flowers</option><option>Puja Supplies</option><option>Incense</option><option>Oil & Lamps</option>
              </optgroup>
              <optgroup label="Equipment">
                <option>Lamps & Deepam</option><option>Vessels & Utensils</option><option>Garments & Vastram</option><option>Sound & AV</option>
              </optgroup>
              <optgroup label="POS / Sale">
                <option>Prasad Packets</option><option>Pooja Kits</option><option>Books & Calendars</option><option>Souvenirs</option>
              </optgroup>
              <optgroup label="Office">
                <option>Stationery</option><option>Cleaning Supplies</option>
              </optgroup>
              <optgroup label="Festival">
                <option>Festival Decorations</option><option>Festival Prasad</option>
              </optgroup>
            </select>
          </div>
          <div className="flex flex-col gap-1">
            ...
            <select className={selectCls}>
              <option>Select after category</option>
              <option>Sweet & cooked</option><option>Dry prasad</option><option>Fruit & nut</option><option>Fire & light</option><option>Sacred powders</option>
            </select>
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>SKU (auto-generated)</label>
            <input className={readonlyCls} value="PRD-244" readOnly />
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Unit of measure *</label>
            <select className={selectCls}>
              <option>Pieces</option><option>Kg</option><option>Grams</option><option>Litres</option><option>Ml</option><option>Packets</option><option>Garlands</option><option>Sticks</option><option>Bunches</option><option>Sets</option>
            </select>
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Used in / for</label>
            <input className={inputCls} placeholder="e.g. Abhishekam, Archana, Deepam" />
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Frequency</label>
            <select className={selectCls}>
              <option>Daily</option><option>Weekly</option><option>Monthly</option><option>Festival only</option><option>One-time</option>
            </select>
          </div>
          <div className="flex flex-col gap-1 col-span-2">
            <label className={labelCls}>Description / notes</label>
            <textarea className={inputCls + " resize-none"} rows={2} placeholder="Storage notes, special handling etc." />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-5">
        ...
        <div className="grid grid-cols-4 gap-3.5">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input className={inputCls} type="number" placeholder="e.g. 50" />
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Unit cost (£)</label>
            <input className={inputCls} placeholder="e.g. 0.45" />
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Sale price (£)</label>
            <input className={inputCls} placeholder="For POS items" />
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Storage location</label>
            <input className={inputCls} placeholder="e.g. Shelf 2A, Storeroom" />
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Pooja BOM linked?</label>
            <select className={selectCls}>
              <option>No — general stock</option><option>Yes — link to pooja type</option>
            </select>
          </div>
          <div className="flex flex-col gap-1">
            <label className={labelCls}>Label type</label>
            <select className={selectCls}>
              <option>Auto barcode</option><option>Auto QR code</option><option>None</option>
            </select>
          </div>
          <div className="flex flex-col gap-1">
