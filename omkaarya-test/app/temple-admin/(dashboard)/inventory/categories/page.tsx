...
import { Plus, Package, Wrench, ShoppingCart, FileBox, PartyPopper, ChevronDown, ChevronRight, Pencil, Trash2 } from "lucide-react";

// ── Types ──────────────────────────────────────────────────────────

type SubCategory = { name: string; count: number };
type Category = { Icon: React.ComponentType<{ className?: string }>; name: string; desc: string; count: number; subs: SubCategory[] };

const INITIAL_CATS: Category[] = [
  { Icon: Package, name: "Consumables", desc: "Items used up and restocked regularly", count: 148, subs: [
  ...
  { Icon: Wrench, name: "Equipment", desc: "Long-term assets tracked by condition and location", count: 42, subs: [
  ...
  { Icon: ShoppingCart, name: "POS / Sale items", desc: "Items sold to devotees at the temple counter", count: 28, subs: [
    { name: "Prasad Packets", count: 8 }, { name: "Pooja Kits", count: 6 }, { name: "Books & Calendars", count: 7 }, { name: "Souvenirs", count: 7 },
  ]},
  { Icon: FileBox, name: "Office & Admin", desc: "Stationery and supplies for temple administration", count: 15, subs: [
    { name: "Stationery", count: 8 }, { name: "Cleaning Supplies", count: 7 },
  ]},
  { Icon: PartyPopper, name: "Festival stock", desc: "Special items ordered for specific festivals", count: 10, subs: [
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-5 gap-2.5">
        {cats.map((c, i) => (
          <div key={i} className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-3">
            <div className="flex items-center gap-1.5 text-[11px] text-zinc-500 dark:text-zinc-400 font-medium"><c.Icon className="w-3.5 h-3.5" />{c.name}</div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        ...
        {cats.map((cat, ci) => (
          <div key={ci}>
            <div className="flex items-center px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors" onClick={() => toggleCat(ci)}>
              <button className={`w-5 h-5 rounded-[5px] border flex items-center justify-center mr-2.5 shrink-0 transition-all ${expanded[ci] ? "bg-[var(--brand-primary)] border-[var(--brand-primary)] text-white" : "border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-500"}`}>
                {expanded[ci] ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
              </button>
              <div className="mr-2.5 shrink-0 text-zinc-500 dark:text-zinc-400"><cat.Icon className="w-5 h-5" /></div>
              ...
              <div className="flex gap-1.5" onClick={e => e.stopPropagation()}>
                <button onClick={() => showToast("Edit")} className="p-1.5 border border-zinc-200 dark:border-zinc-700 rounded-md text-zinc-500 dark:text-zinc-400 hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)] transition-colors"><Pencil className="w-3 h-3" /></button>
                <button onClick={() => showToast("Confirm delete?")} className="p-1.5 border border-zinc-200 dark:border-zinc-700 rounded-md text-zinc-500 dark:text-zinc-400 hover:border-red-400 hover:text-red-500 transition-colors"><Trash2 className="w-3 h-3" /></button>
              </div>
            </div>
            ...
            {expanded[ci] && cat.subs.map((sub, si) => (
              <div key={si} className="flex items-center py-2.5 pl-[52px] pr-4 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/20 hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors cursor-pointer">
                ...
                <div className="flex gap-1.5">
                  <button onClick={() => showToast("Edit sub-category")} className="p-1.5 border border-zinc-200 dark:border-zinc-700 rounded-md text-zinc-500 dark:text-zinc-400 hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)] transition-colors"><Pencil className="w-3 h-3" /></button>
                  <button onClick={() => showToast("Delete?")} className="p-1.5 border border-zinc-200 dark:border-zinc-700 rounded-md text-zinc-500 dark:text-zinc-400 hover:border-red-400 hover:text-red-500 transition-colors"><Trash2 className="w-3 h-3" /></button>
                </div>
              </div>
            ))}
            ...
            {expanded[ci] && addingTo === ci && (
              <div className="flex items-center gap-2 py-2 pl-[52px] pr-4 border-t border-dashed border-zinc-200 dark:border-zinc-700 bg-zinc-50/30 dark:bg-zinc-800/10">
                ...
                <button onClick={() => setAddingTo(null)} className="px-2.5 py-1.5 rounded-md border border-zinc-200 dark:border-zinc-700 text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 transition-colors">Cancel</button>
              </div>
            )}

            {expanded[ci] && addingTo !== ci && (
              <div className="pl-[52px] pr-4 py-2 border-t border-zinc-100 dark:border-zinc-800">
                <button onClick={() => openAdd(ci)} className="inline-flex items-center gap-1 px-2 py-1 text-[11px] border border-orange-200 dark:border-orange-800/50 rounded-md text-[var(--brand-primary)] bg-orange-50 dark:bg-orange-950/30 hover:bg-orange-100 transition-colors font-semibold">
