...
type PoojaBooking = { id: string; label: string; items: ReturnItem[] };

const BOOKINGS: PoojaBooking[] = [
  {
    id: "P142", label: "POOJA-0142 — Rudrabhishekam — Rajan Kumar — Today 07:00",
    items: [
      { Icon: Flame, name: "Camphor Tablets", sku: "PJA-001", unit: "Packets", issued: 3, stock: 0 },
      { Icon: Droplets, name: "Sesame Oil", sku: "OIL-001", unit: "Litres", issued: 2, stock: 18 },
      { Icon: Flower2, name: "Rose Garland", sku: "FLW-001", unit: "Garlands", issued: 4, stock: 12 },
      { Icon: Flower2, name: "Tulsi Leaves", sku: "FLW-003", unit: "Bunches", issued: 2, stock: 8 },
      { Icon: Circle, name: "Kumkum Powder", sku: "PJA-002", unit: "Packets", issued: 1, stock: 25 },
    ],
  },
  {
    id: "P141", label: "POOJA-0141 — Archana — Priya Sharma — Today 09:30",
    items: [
      { Icon: Flame, name: "Camphor Tablets", sku: "PJA-001", unit: "Packets", issued: 1, stock: 0 },
      { Icon: Circle, name: "Kumkum Powder", sku: "PJA-002", unit: "Packets", issued: 1, stock: 25 },
      { Icon: Flower2, name: "Rose Garland", sku: "FLW-001", unit: "Garlands", issued: 1, stock: 12 },
    ],
  },
  {
    id: "P138", label: "POOJA-0138 — Homam — Sri Murugan Trust — 18 Apr",
    items: [
      { Icon: Flame, name: "Ghee (cow)", sku: "OIL-002", unit: "Litres", issued: 2, stock: 4 },
      { Icon: Flame, name: "Camphor Tablets", sku: "PJA-001", unit: "Packets", issued: 5, stock: 0 },
      { Icon: Droplets, name: "Sesame Oil", sku: "OIL-001", unit: "Litres", issued: 3, stock: 18 },
      { Icon: Flower2, name: "Rose Garland", sku: "FLW-001", unit: "Garlands", issued: 8, stock: 12 },
    ],
  },
];
...
export default function ReturnFromPoojaPage() {
  ...
  const handleSelectBooking = (val: string) => { setSelectedBooking(val); setShowItems(!!val); setShowSuccess(false); };
  const handleClear = () => { setSelectedBooking(""); setShowItems(false); setShowSuccess(false); };
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Return from Pooja</h1><p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">Return unused items from a completed pooja back to stock</p></div>
      ...
      <div className="flex gap-2.5 items-start bg-purple-50 dark:bg-purple-950/30 border-[1.5px] border-purple-200 dark:border-purple-800/50 rounded-xl p-3">
        ...
        <div>
          ...
          <div className="text-[11px] text-zinc-600 dark:text-zinc-400 leading-relaxed">Items returned here go back to stock. The income from the original pooja booking is <strong>not reversed</strong>. If you need to refund money to a devotee, go to Finance → Add Transaction → Income reversal.</div>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden mb-4">
            <div className="flex items-center px-4 py-2.5 bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-700 text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">
              <div className="flex-1">Item</div><div className="w-[100px] text-center">Qty issued</div><div className="w-[120px] text-center">Returning qty</div><div className="w-[100px] text-center">Stock after</div>
            </div>
            {booking.items.map((item, i) => (
              <div key={i} className="flex items-center px-4 py-2.5 border-b border-zinc-100 dark:border-zinc-800 last:border-b-0">
                <div className="flex items-center gap-2 flex-1">
                  <div className="w-[30px] h-[30px] rounded-lg bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-zinc-500 dark:text-zinc-400 shrink-0"><item.Icon className="w-3.5 h-3.5" /></div>
                  <div><div className="text-xs font-semibold text-zinc-900 dark:text-zinc-100">{item.name}</div><div className="text-[10px] text-zinc-400 font-mono">{item.sku} · {item.unit}</div></div>
                </div>
                <div className="w-[100px] text-center text-[11px] text-zinc-500">Issued: <strong className="text-zinc-900 dark:text-zinc-100">{item.issued}</strong></div>
                <div className="w-[120px] text-center"><input type="number" defaultValue={0} min={0} max={item.issued} className="w-[80px] border border-zinc-200 dark:border-zinc-700 rounded-md px-2 py-1.5 text-xs font-[inherit] text-center outline-none bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 focus:border-[var(--brand-primary)] transition-colors" /></div>
                <div className="w-[100px] text-center text-[11px] text-zinc-500">Current: {item.stock}</div>
              </div>
            ))}
          </div>
      ...
      {showSuccess && (
        <div>
          <div className="flex gap-2.5 items-start bg-emerald-50 dark:bg-emerald-950/30 border-[1.5px] border-emerald-200 dark:border-emerald-800/50 rounded-xl p-3">
            ...
            <div><div className="text-xs font-bold text-emerald-800 dark:text-emerald-300 mb-1">Items returned to stock successfully</div><div className="text-[11px] text-zinc-600 dark:text-zinc-400">Items have been added back to inventory. No financial entry was created. Reference: RTRN-0042</div></div>
          </div>
          <button onClick={() => router.push("/temple-admin/inventory/adjustments")} className="mt-3 inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)] transition-colors"><ArrowLeft className="w-3.5 h-3.5" />Back to adjustments</button>
        </div>
      )}
