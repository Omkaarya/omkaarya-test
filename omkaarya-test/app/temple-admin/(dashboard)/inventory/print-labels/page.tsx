...
const INITIAL_ITEMS: PrintItem[] = [
  { Icon: Cookie, name: "Besan Ladoo", sku: "PRD-001", selected: true, copies: 1 },
  { Icon: Cookie, name: "Motichoor Ladoo", sku: "PRD-002", selected: true, copies: 1 },
  { Icon: Cookie, name: "Coconut Ladoo", sku: "PRD-003", selected: true, copies: 1 },
  { Icon: Flower2, name: "Rose Garland", sku: "FLW-001", selected: false, copies: 1 },
  { Icon: Flower2, name: "Marigold Loose", sku: "FLW-002", selected: false, copies: 1 },
  { Icon: Flame, name: "Camphor Tablets", sku: "PJA-001", selected: false, copies: 1 },
  { Icon: Droplets, name: "Sesame Oil", sku: "OIL-001", selected: false, copies: 1 },
  { Icon: Wind, name: "Sandalwood Incense", sku: "INC-001", selected: false, copies: 1 },
];
...
export default function PrintLabelsPage() {
  ...
  const toggleItem = (i: number) => setItems(prev => prev.map((item, idx) => idx === i ? { ...item, selected: !item.selected } : item));
  const adjustQty = (i: number, delta: number) => setItems(prev => prev.map((item, idx) => idx === i ? { ...item, copies: Math.max(1, item.copies + delta) } : item));
  const selectAll = () => setItems(prev => prev.map(item => ({ ...item, selected: true })));

  const filteredItems = useMemo(() => {
    const q = search.toLowerCase();
    return items.map((item, origIdx) => ({ ...item, origIdx })).filter(item => !q || item.name.toLowerCase().includes(q) || item.sku.toLowerCase().includes(q));
  }, [items, search]);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div><h1 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Print Labels</h1><p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">Generate barcode or QR code labels for inventory items</p></div>
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={() => showToast("Downloading PDF...")} className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)] transition-colors"><Download className="w-3.5 h-3.5" />PDF</button>
          <button onClick={() => showToast("Printing...")} className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--brand-primary)] px-3 py-2 text-[11px] font-semibold text-white hover:bg-[var(--brand-primary-hover)] transition-colors"><Printer className="w-3.5 h-3.5" />Print selected</button>
        </div>
      </div>

      <div className="flex bg-zinc-100 dark:bg-zinc-800 rounded-lg p-0.5 w-fit border border-zinc-200 dark:border-zinc-700">
        <button onClick={() => setLabelMode("bc")} className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-md text-xs font-medium transition-all ${labelMode === "bc" ? "bg-white dark:bg-zinc-900 text-[var(--brand-primary)] font-bold shadow-sm" : "text-zinc-500 dark:text-zinc-400"}`}><Barcode className="w-3.5 h-3.5" />Barcode</button>
        <button onClick={() => setLabelMode("qr")} className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-md text-xs font-medium transition-all ${labelMode === "qr" ? "bg-white dark:bg-zinc-900 text-[var(--brand-primary)] font-bold shadow-sm" : "text-zinc-500 dark:text-zinc-400"}`}><QrCode className="w-3.5 h-3.5" />QR Code</button>
      </div>
            ...
            {filteredItems.map((item) => (
              <div key={item.origIdx} className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-100 dark:border-zinc-800 last:border-b-0">
                <div className="flex items-center gap-2">
                  ...
                  <div className="w-[26px] h-[26px] rounded-md bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-zinc-500 dark:text-zinc-400"><item.Icon className="w-3 h-3" /></div>
                  <div><div className="text-[11px] font-semibold text-zinc-900 dark:text-zinc-100">{item.name}</div><div className="text-[10px] text-zinc-400 font-mono">{item.sku}</div></div>
                </div>
                <div className="flex items-center gap-1.5">
                  <button onClick={() => adjustQty(item.origIdx, -1)} className="w-[26px] h-[26px] border border-zinc-200 dark:border-zinc-700 rounded-md bg-white dark:bg-zinc-900 flex items-center justify-center text-zinc-500 hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)] transition-colors"><Minus className="w-3 h-3" /></button>
                  <span className="text-xs font-bold w-5 text-center text-zinc-900 dark:text-zinc-100">{item.copies}</span>
                  <button onClick={() => adjustQty(item.origIdx, 1)} className="w-[26px] h-[26px] border border-zinc-200 dark:border-zinc-700 rounded-md bg-white dark:bg-zinc-900 flex items-center justify-center text-zinc-500 hover:border-[var(--brand-primary)] hover:text-[var(--brand-primary)] transition-colors"><Plus className="w-3 h-3" /></button>
                </div>
              </div>
            ))}
          </div>
              ...
              {selectedItems.length === 0 ? (
                ...
                selectedItems.slice(0, 4).map((item, i) => (
                  <div key={i} className="border-[1.5px] border-zinc-200 dark:border-zinc-700 rounded-lg p-3 flex flex-col items-center min-w-[100px] bg-white dark:bg-zinc-900">
                    ...
                    {labelMode === "bc" ? <BarcodePreview /> : <QRPreview />}
