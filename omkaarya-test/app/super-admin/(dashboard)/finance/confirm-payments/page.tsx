...
import { dispatchPendingPaymentSubmissionsChanged } from "@/lib/pending-payment-submissions-events";
...
function Toast({
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-[200] flex items-center gap-3 rounded-xl border px-5 py-4 shadow-xl ${type === "success" ? "border-success-500/20 bg-status-success-bg text-status-success-text" : "border-error-500/20 bg-status-danger-bg text-status-danger-text"}`}
    >
      {type === "success" ? (
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-text-tertiary">
          Temples that have submitted bank transfer notifications — verify and
          activate
        </p>
      </div>
      ...
      <div className="rounded-xl border-[1.5px] border-amber-300 bg-amber-50 dark:bg-amber-950/20 p-4 flex gap-3 items-start">
        ...
        <div>
          ...
          <p className="text-xs text-text-secondary leading-relaxed">
            ...
            you receive a notification here. Verify the bank transfer, then
            click Confirm to activate their portal and auto-generate a receipt.
          </p>
        </div>
      </div>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex gap-3 items-start flex-1">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-brand-50 border border-brand-100 text-xl">
                  🛕
                </div>
                <div className="flex-1">
                  ...
                  <p className="text-xs text-text-tertiary mb-3">
                    {payment.location} · {payment.invoiceId ?? "—"} ·{" "}
                    {payment.plan} plan
                  </p>
                  ...
                  <div className="grid grid-cols-4 gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p
                        className={`text-xs font-semibold ${payment.slipUploaded ? "text-green-600" : "text-red-600"}`}
                      >
                  ...
                  {payment.slipUploaded && payment.slipUrl ? (
                    ...
                    <p className="text-xs text-amber-600">
                      ⚠️ Temple did not upload a payment slip — verify bank
                      statement directly
                    </p>
                  )}
                </div>
