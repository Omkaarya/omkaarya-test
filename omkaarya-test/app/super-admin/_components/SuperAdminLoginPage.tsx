  ...
  useEffect(() => {
    try {
      if (
        ...
        localStorage.getItem(SUPER_ADMIN_REMEMBER_ME_STORAGE_KEY) === "1"
      ) {
        setRememberMe(true);
      }
    } catch {
  ...
  useEffect(() => {
    const redirectIfAuthenticated = async () => {
      try {
        ...
        if (res.ok) window.location.replace("/super-admin/dashboard");
      } catch {
        /* stay on login */
      }
    };
    ...
    const onPageShow = (event: PageTransitionEvent) => {
      if (event.persisted) void redirectIfAuthenticated();
    };
      ...
      if (response.ok) {
        try {
          localStorage.setItem(
            SUPER_ADMIN_REMEMBER_ME_STORAGE_KEY,
            rememberMe ? "1" : "0",
          );
        } catch {
          /* ignore */
        ...
        <div className="relative z-10 flex flex-col items-center gap-3 text-center">
          ...
          <div>
            ...
            <p className="mt-3 max-w-md text-lg font-medium text-white/90 lg:text-xl">
              Platform administration for temples, subscriptions, and system
              settings.
            </p>
          </div>
        </div>
