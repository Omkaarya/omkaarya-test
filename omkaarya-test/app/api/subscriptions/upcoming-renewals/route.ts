...
export async function GET(request: NextRequest) {
  try {
    ...
    const target = `${apiUrl("/api/subscriptions/upcoming-renewals")}${request.nextUrl.search}`;
    const res = await fetch(target, { method: "GET", headers: auth.headers, cache: "no-store" });
    const data = await res.json().catch(() => null);
    return NextResponse.json(data, { status: res.status });
  } catch (e) {
    const message = e instanceof Error ? e.message : "Failed to load upcoming renewals";
