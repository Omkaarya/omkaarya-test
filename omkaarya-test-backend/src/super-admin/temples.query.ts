...
export function parseTemplesQuery(searchParams: URLSearchParams): TemplesQueryInput {
  ...
  const rawSort = (searchParams.get("sortBy") ?? "timeline").trim().toLowerCase();
  const normalizedSort: TemplesSortBy =
    rawSort === "last7" || rawSort === "timeline" ? "timeline" : (rawSort as TemplesSortBy);
  const sortBy = ALLOWED_SORTS.includes(normalizedSort) ? normalizedSort : "timeline";
...
export function filterTemples(rows: TempleRecord[], query: TemplesQueryInput): TempleRecord[] {
  let filtered = [...rows];
...
export function sortTemples(rows: TempleRecord[], sortBy: TemplesSortBy): TempleRecord[] {
  const sorted = [...rows];
...
export function paginateTemples(rows: TempleRecord[], page: number, pageSize: number) {
...
export function listCountries(rows: TempleRecord[]): string[] {
